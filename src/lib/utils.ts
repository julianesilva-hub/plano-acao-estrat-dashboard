...
/**
 * PROBLEMA 4: Status exibido automaticamente.
 * Se prazo < hoje e status != Concluído → exibe "Atrasado"
 * Não altera o dado original, apenas o que é exibido na tabela.
 */
export function getDisplayStatus(r: Acao): string {
  if (!isConcluido(r.Status) && r._prazo !== null && r._prazo < today()) {
    return 'Atrasado'
  }
  return r.Status
}
