import { Download, FileSpreadsheet, Search, ChevronUp, ChevronDown, ChevronsUpDown, Pencil } from 'lucide-react'
...
export function Table({
  ...
  const pageBtns = Array.from({ length: Math.min(totalPages, maxPageBtns) }, (_, i) => i + 1)
...
function TableRow({ row, onEdit }: { row: Acao; onEdit: (row: Acao) => void }) {
  ...
  return (
    <tr className="hover:bg-gray-50 border-b border-gray-50 last:border-0">
      ...
      <td className="px-3 py-2 overflow-hidden text-ellipsis whitespace-nowrap text-gray-500" title={row['Observação']}>
