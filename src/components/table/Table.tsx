...
import { useUser } from '../../context/UserContext'   // "Você é" — independente de Responsável
        ...
        <table className="w-full text-xs border-collapse" style={{ tableLayout: 'fixed' }}>
          ...
          <tbody>
            {paged.length === 0
              ...
              : paged.map((r, i) => (
                  <TableRow key={r.ID || `row-${i}`} row={r} onEdit={onEdit} />
                ))
            }
          </tbody>
        </table>
...
function TableRow({ row, onEdit }: { row: Acao; onEdit: (row: Acao) => void }) {
  const { currentUser } = useUser()   // "Você é" — INDEPENDENTE do filtro Responsável

  const dd  = row._prazo ? diffDays(row._prazo) : null
  const atr = isAtrasada(row) && dd !== null
  const sc  = statusStyle(row.Status)

  // Permissão baseada APENAS em "Você é" — não em filtro Responsável
  const canEdit = currentUser !== '' && row['Responsável'] === currentUser
  ...
  return (
    <tr className="hover:bg-gray-50 border-b border-gray-50 last:border-0">
      ...
      <td className="px-3 py-2 overflow-hidden text-ellipsis whitespace-nowrap text-gray-500"
      ...
      <td className="px-3 py-2">
        {canEdit ? (
          ...
          <span title={currentUser ? 'Você não é o responsável' : 'Selecione "Você é" para editar'}>
            <Lock size={11} className="text-gray-200 mx-auto block" />
          </span>
        )}
      </td>
    </tr>
